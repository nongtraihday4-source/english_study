...
class PasswordResetConfirmView(APIView):
    ...
    Validates the uid + token (generated by support staff via SupportPasswordResetView)
    ...
    def post(self, request):
        ...
        if not uid_b64 or not token or not new_password:
            return Response(
                {"detail": "uid, token và new_password là bắt buộc."},
        ...
        try:
            ...
            user = User.objects.get(pk=user_pk, is_deleted=False)
        ...
        auth_logger.info("Password reset confirmed | user=%s", user.pk)
        return Response({"detail": "Đặt lại mật khẩu thành công. Bạn có thể đăng nhập ngay."})
